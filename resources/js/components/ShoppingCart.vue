                ...
                <table class="w-full text-sm lg:text-base" cellspacing="0">
                    ...
                    <tbody>
                        <tr v-for="p in products">
                            ...
                            <td>
                                <a href="#">
                                    <p class="mb-2 md:ml-4">{{ p.name }}</p>
                                    <form action="" method="POST">
                                        <button
                                            type="submit"
                                            @click.prevent="destroy(p.id)"
                            ...
                            >
                                <div class="w-20 h-10">
                                    ...
                                    >
                                        <button
                                            class="border border-indigo-600 px-3 text-xl font-bold text-indigo-600 bg-white transition duration-300 hover:bg-indigo-600 hover:text-white rounded-l"
                                            @click.prevent="increase(p.id)"
                                        >
                                            +
                                        </button>
                                        <input
                                            type="number"
                                            :value="p.quantity"
                                            class="w-16 h-8 font-semibold text-center text-indigo-600 bg-indigo-100 border-t border-b border-indigo-600 outline-none focus:outline-none"
                                        />
                                        <button
                                            @click.prevent="decrease(p.id)"
                                            class="border border-indigo-600 px-3 text-xl font-bold text-indigo-600 bg-white transition duration-300 hover:bg-indigo-600 hover:text-white rounded-r"
                                        >
                                            -
                                        </button>
                                    </div>
                            ...
                            <td class="text-right">
                                <span
                                    class="text-sm lg:text-base font-medium"
                                    v-text="formatPrice(p.price * p.quantity)"
                                >
                                </span>
                            </td>
                        </tr>
...
const increase = async (id) => {
    ...
    await getProducts();
    eventBus.value.$emit("countUpdated", cartCount.value);
};
